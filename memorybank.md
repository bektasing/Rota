...
**Mevcut phase:** Android APK paketleme (Capacitor) — TYT–AYT kataloğu/Hızlı Planla sonrası  
**Durum:** Tamamlandı — web uygulaması değişmeden Capacitor ile Android'e paketlendi, debug APK üretildi ve gerçek emülatörde uçtan uca doğrulandı; imzalı release APK için kullanıcının kendi keystore adımı bekleniyor  

**Android APK paketleme — Capacitor entegrasyonu (bu geçiş):**
- Mevcut React + TypeScript + Vite web uygulaması **hiç yeniden yazılmadan** Capacitor 8.5.0 ile Android'e paketlendi. Eklenen paketler: `@capacitor/core`, `@capacitor/app` (runtime bağımlılık); `@capacitor/cli`, `@capacitor/android`, `@capacitor/filesystem`, `@capacitor/share`, `@capacitor/status-bar`, `@capacitor/splash-screen`, `@capacitor/assets` (devDependency — sonuncusu yalnızca ikon/splash üretimi için build-time aracı). `capacitor.config.ts`: `appId: "com.rota.nisa"`, `appName: "Rota"`, `webDir: "dist"`, `server.url` **yok** (APK tamamen yerel dosyalardan çalışıyor, uzak sunucuya bağımlı değil).
- `android/` klasörü `npx cap add android` ile oluşturuldu, `npx cap sync android` ile web build'i ve 5 plugin (`app`, `filesystem`, `share`, `splash-screen`, `status-bar`) native projeye senkronize edildi. **Ortam notu:** sistemdeki varsayılan Java (25) Gradle 8.14.3 ile uyumsuz çıktı ("Unsupported class file major version 69"); çözüm olarak Android Studio'nun kendi JBR'ı (Java 21, `/Applications/Android Studio.app/Contents/jbr/Contents/Home`) `JAVA_HOME` olarak kullanıldı — bu bilgi `ANDROID_APK.md`'ye de eklendi.
- **Uygulama kimliği:** ad "Rota", paket adı `com.rota.nisa`, dil Türkçe (mevcut arayüz zaten Türkçe), yön `android:screenOrientation="portrait"` ile sabitlendi, `minSdkVersion=24`/`targetSdkVersion=36` (Capacitor 8 varsayılanları, elle düşürülmedi/yükseltilmedi). `versionName="1.0.0"`, `versionCode=1`. AndroidManifest'te yalnızca `INTERNET` izni var (Capacitor'ın yerel WebView köprüsü için gerekli, gerçek internet erişimi anlamına gelmiyor) — kamera/konum/mikrofon/bildirim/kişi izni **eklenmedi**.
- **İkon ve splash:** Mevcut PWA ikonu (`public/pwa-512x512.png`, 512×512, mor-lila marka rengi) `@capacitor/assets` Easy Mode ile launcher/round/adaptive icon ve splash screen'e dönüştürüldü (`assets/logo.png` kaynak, `--iconBackgroundColor`/`--splashBackgroundColor` mevcut CSS tasarım tokenlarından alındı: açık `#f8f6f3`, koyu `#101320`). **Bilinen kısıt:** kaynak 512×512, `@capacitor/assets`'in tam kontrol modu için önerdiği 1024×1024'ün altında; splash için `logoSplashScale=0.22` ile logo ekranın küçük bir kısmında tutularak zorla büyütmenin görünür bulanıklığı en aza indirildi (splash'i tam ekran kaplayacak şekilde büyütmek yerine). Launcher ikonu için 512×512 fazlasıyla yeterli (Android en yüksek yoğunlukta 192×192 istiyor).
- **Android geri tuşu:** Büyük bir navigasyon refactor'ı yapılmadan hafif bir çözüm eklendi: `src/utils/nativeBackStack.ts` (açık panel/dialog'ların kayıt olduğu basit bir yığın), `FormSheet.tsx` ve `ConfirmDialog.tsx` artık mount olduklarında kendi `onClose`/`onCancel`'larını bu yığına kaydediyor (tüm formlar ve onay pencereleri otomatik kapsanıyor, tek tek sayfa değişikliği gerekmedi). `src/services/androidBackButton.ts`, yalnızca native ortamda `@capacitor/app`'in `backButton` olayını dinliyor: önce açık panel varsa kapatıyor, yoksa route geçmişinde geri gidiyor (`router.navigate(-1)`, `history.state.idx` ile geçmiş olup olmadığı kontrol ediliyor), kök sayfadaysa `App.exitApp()` ile uygulamadan çıkıyor. Gerçek emülatörde canlı doğrulandı: TYT Matematik konu sayfasındayken donanım geri tuşu, doğru şekilde bir üstteki Dersler listesine döndü.
- **Native yedekleme:** `src/services/nativeBackup.ts` (`saveOrShareBackupNative`) yedek JSON'ını `@capacitor/filesystem` ile uygulamanın önbellek alanına yazıp `@capacitor/share` ile Android paylaşım ekranını açıyor (Dosyalar/Drive/WhatsApp vb. hedeflere kaydedilebiliyor); FileProvider zaten Capacitor'ın varsayılan `file_paths.xml`'inde (`cache-path`) tanımlı, ek yapılandırma gerekmedi. `BackupCard.tsx`, `Capacitor.isNativePlatform()` kontrolüyle native'de bu akışı, web'de mevcut Blob+anchor indirmeyi kullanıyor; buton metni native'de "Yedeği kaydet / paylaş" olarak değişiyor. Dosya adı korundu: `rota-yedek-YYYY-AA-GG.json`. İçe aktarma (`<input type="file">`) değiştirilmedi — Android WebView'da ek bir plugin gerektirmeden çalıştığı standart Capacitor davranışıdır; gerçek cihazda dosya seçici arayüzü ayrıca interaktif test edilmedi (yalnızca kod incelemesi + Capacitor'ın resmi desteği).
- **Service worker:** `vite-plugin-pwa`'nın koşulsuz enjekte ettiği `registerSW.js` üretilen dosyalara dokunulmadan devre dışı bırakıldı: `src/main.tsx`'in en başında, yalnızca `Capacitor.isNativePlatform()` doğruysa `navigator.serviceWorker.register` no-op bir fonksiyonla değiştiriliyor (window `load` olayından önce çalıştığı için zamanlama güvenli). Web/PWA sürümü bu satırda hiç etkilenmiyor (native değilken kod bloğu hiç çalışmıyor) — `npm run build` çıktısı hâlâ `sw.js`/`workbox-*.js` üretiyor ve tarayıcıda PWA davranışı değişmedi.
- **Edge-to-edge, safe-area, status bar:** `src/services/androidStatusBar.ts`, native'de `StatusBar.setOverlaysWebView({overlay: true})` ile edge-to-edge açıyor ve `document.documentElement`'teki `dark` sınıfını bir `MutationObserver` ile izleyip durum çubuğu simge rengini (`Style.Light`/`Style.Dark`) temaya göre canlı güncelliyor. Edge-to-edge içeriğin durum çubuğunun altına girmemesi için `TopBar.tsx`'e `pt-[env(safe-area-inset-top)]` eklendi (BottomNav'da eşdeğeri zaten vardı: `pb-[env(safe-area-inset-bottom)]`). Klavye açıldığında FormSheet'in kaydet butonunun erişilebilir kalması için `AndroidManifest.xml`'e `android:windowSoftInputMode="adjustResize"` eklendi. Gerçek emülatörde açık ve koyu temada canlı doğrulandı: durum çubuğu içeriğin üstüne binmiyor, tema değişince simge rengi doğru güncelleniyor.
- **Debug APK:** `cd android && ./gradlew assembleDebug` ile başarıyla üretildi → `android/app/build/outputs/apk/debug/app-debug.apk` (4,8 MB). `aapt2 dump badging` ile paket kimliği/sürüm/izinler doğrulandı (`com.rota.nisa`, versionCode 1, versionName 1.0.0, yalnızca `INTERNET` izni).
- **Gerçek Android emülatöründe uçtan uca smoke test** (Medium_Phone AVD, Android API 36, temiz/izole veri): kurulum → onboarding (günlük hedef 180 dk, TYT/AYT Orta seviye) tamamlandı ve IndexedDB'ye yazıldı → Dashboard gerçek verilerle açıldı ("İyi akşamlar, Nisa", "YKS'ye 319 gün kaldı", 0/180 dk hedef halkası) → Dersler ve Konular ekranında TYT/AYT/Özel geçişi çalıştı, **TYT Matematik'te tam 25 konu** (kataloğun beklediği sayı) doğru sırayla listelendi ("Temel Kavramlar", "Sayı Basamakları", "Bölme ve Bölünebilme", "EBOB-EKOK"...) → donanım geri tuşu konu sayfasından ders listesine doğru döndü → koyu tema açıldı, tüm ekran (kartlar, rozetler, durum çubuğu simgeleri) doğru şekilde koyu temaya geçti, kontrast korundu. Test emülatörü ayrı/izole bir cihazdı, gerçek geliştirme verisine (`localhost:5173`) hiç dokunulmadı; test sonunda emülatör kapatıldı, ek temizlik gerekmedi.
- **Yapılmayanlar (kapsam dışı, brief'e uygun):** Vercel/hosting/backend/Firebase/Supabase eklenmedi, Flutter/React Native'e geçilmedi, SQLite migrasyonu yapılmadı, Play Store'a yayın yapılmadı, AAB zorunlu kılınmadı, bildirim sistemi eklenmedi, hiçbir git işlemi (commit/push/branch/tag) yapılmadı.
- **Bilinen sorunlar (bu geçişten):** İkon/splash kaynağı 512×512 (1024×1024 önerilenin altında) — launcher ikonu için sorun değil, splash'te küçük ölçekte (0.22) tutularak etkisi en aza indirildi ama en büyük ekranlarda hafif yumuşaklık olabilir; daha keskin bir görünüm için ileride 1024×1024+ bir kaynak PNG sağlanabilir. Backup **içe aktarma** akışı (`<input type="file">`) gerçek cihazda interaktif olarak test edilmediğinden yalnızca Capacitor'ın resmi desteğine ve kod incelemesine dayanıyor. Release imzalama bilinçli olarak tamamlanmadı — keystore oluşturma ve imzalama adımları `ANDROID_APK.md`'de anlatıldığı gibi kullanıcının kendisi tarafından Android Studio üzerinden yapılmalı (parola projeye asla gömülmedi). `npm audit`, `@capacitor/assets`'in geçişli bağımlılıklarında (yalnızca build-time ikon üretim aracı, uygulamaya paketlenmiyor) bazı uyarılar gösteriyor; bunlar üretim JS bundle'ına dahil değil.

**TYT–AYT konu kataloğu, Hızlı Planla ve Nisa'ya özel UI:**
- Kullanıcının verdiği iki referans görselden (TYT ve AYT Sayısal konu tabloları) 246 konu elle çıkarıldı ve `src/constants/topicCatalog.ts` içinde merkezi, tip güvenli bir katalog olarak tutuldu (`{examType, subjectName, topicName, order}`). Görsellerde okunabilen metin birebir korundu; üç yerde kaynağa sadık kalınarak aktarılan olası kaynak yazım tuhaflıkları koda yorum olarak not edildi: TYT Geometri 21. madde "Küre" ifadesini iki kez içeren birleşik bir satır; TYT Kimya 6. madde "Kimsayal" (muhtemelen "Kimyasal" yazım hatası); AYT Geometri 3. madde "Trigonemetrik" (muhtemelen "Trigonometrik"); TYT Tarih 17. madde "...Hazırlık D." kısaltmasıyla bitiyor. Hiçbir konu tahmin edilmedi, internetten müfredat araştırması yapılmadı, katalog "resmî müfredat" olarak sunulmuyor.
- `src/services/topicCatalogService.ts` (`ensureTopicCatalogSeeded`) kataloğu idempotent şekilde ekliyor: dersleri hem sınav türü hem ada göre eşleştiriyor (aynı isimli TYT/AYT dersleri karışmıyor), her ders için mevcut konuları tek seferde okuyup normalize edilmiş adla karşılaştırıyor, yalnızca eksik konuları ekliyor; hiçbir konu silinmiyor/güncellenmiyor, sıra numaraları mevcut maksimumdan devam ediyor. Gerçek geliştirme verisinde (`localhost:5173`) canlı doğrulandı: TYT Matematik'te kullanıcının önceden eklediği tek özel konu ("Türev") ve durumu dokunulmadan korundu, kataloğun 25 TYT Matematik konusu eklenerek toplam 26 oldu; tüm 15 varsayılan ders için IndexedDB'den okunan konu sayıları kataloğun beklediği sayılarla birebir eşleşti (ör. AYT Matematik 31, AYT Geometri 27, TYT Geometri 21); sayfa yenilenip seed ikinci kez çalıştığında toplam konu sayısı (247) değişmedi — çoğalma yok. Seed, `AppShell` içinde uygulama genelinde tek noktadan tetikleniyor; ayrıca `TopicsPage` ve `QuickPlanPanel` kendi konu listelerini yüklemeden önce de çağırıyor (deep-link ile doğrudan bir dersin konu sayfasına gelindiğinde de güvenli).
- **Dersler ve Konular ekranı:** `SubjectsPage`'e TYT/AYT Sayısal/Özel arasında hızlı geçiş için bir `SegmentedControl` eklendi (varsayılan TYT). `TopicsPage`'e: toplam/tamamlanan/çalışılıyor/tekrar-gerekli özet sayaçları, durum filtresi (Tümü/Başlanmadı/Çalışılıyor/Tamamlandı/Tekrar gerekli — `SegmentedControl` ile), ve her konu kartına bir "Planla" aksiyonu (`CalendarPlus` ikonu) eklendi. Mevcut konu ekleme/adını düzenleme/silme/durum değiştirme davranışı değişmedi.
- **Hızlı Planla akışı** (`src/features/planner/components/QuickPlanPanel.tsx`): Plan ekranında mevcut ayrıntılı görev formunun yanına ikinci, dokunmatik-öncelikli bir ekleme yolu eklendi. Akış: sınav türü (TYT/AYT/Özel — brief'in iki seçenekli "TYT/AYT seç" adımına, mevcut Özel ders özelliğini ikinci sınıfa düşürmemek için bilinçli olarak üçüncü bir "Özel" sekmesi eklendi, bkz. bilinen sorunlar) → ders (yalnızca aktif dersler, native seçici) → konu (isteğe bağlı, native seçici) → çalışma türü (6 dokunmatik çip: Konu çalış/Soru çöz/Tekrar yap/Branş denemesi/Genel deneme/Yanlış analizi — doğrudan mevcut `TaskType` değerlerine eşleniyor) → gün (Bugün/Yarın/Hafta sonu/Tarih seç) → süre veya soru hedefi (soru çözümü seçiliyse soru hedefi çipleri, diğer tüm türlerde süre çipleri — konu çalışmasında soru hedefi hiç gösterilmiyor) → otomatik başlık (düzenlenebilir) → Kaydet.
- **Otomatik başlık** (`src/utils/quickPlan.ts`, `buildAutoTitle`): ders/konu/çalışma türüne göre `"{TYT/AYT} {Ders} · {Konu} çalış"`, `"...tekrarı"`, `"...{N} soru çöz"` gibi başlıklar üretiyor; soru çözümü için basit bir Türkçe ünlü uyumu fonksiyonuyla konu adına doğru ayrılma durumu eki (-den/-dan/-ten/-tan) ekleniyor (ör. "Problemler" → "Problemlerden"). Kullanıcı başlığı elle değiştirdiği an otomatik üretim o panel açıkken bir daha başlığı ezmiyor (`titleTouched` bayrağı). Branş denemesi/genel deneme/yanlış analizi için örnek verilmediğinden makul, tutarlı bir varsayım uygulandı (bkz. bilinen sorunlar).
- **Hafta sonu hesaplama** (`computeNextWeekend`): bugünden kesinlikle sonraki en yakın Cumartesi'yi yerel tarih bileşenleriyle (UTC kayması olmadan, mevcut `addDays` yardımcısı üzerinden) hesaplıyor; bugün Cumartesi ise bir hafta sonrasına atlıyor.
- **Konudan doğrudan planlama:** `TopicsPage`'deki "Planla" aksiyonu `buildQuickPlanSearch` ile `/plan?quick=1&examType=..&subjectId=..&topicId=..` sorgu parametreleri üretip yönlendiriyor; `PlannerPage` bunu `parseQuickPlanDeepLink` ile okuyup Hızlı Planla panelini ön-dolu açıyor ve parametreleri `replace: true` ile temizliyor (yenilemede tekrar açılmıyor). Yeni bir global state sistemi kurulmadı; yalnızca URL sorgu parametreleri ve bir bileşen prop'u kullanıldı. **Bulunan ve düzeltilen gerçek bir hata:** derin bağlantı, dersler henüz `PlannerPage`'in kendi `useEffect`'inde yüklenmeden paneli açıyordu; bu yüzden panel ders/konu ön-seçimini kaybediyordu (boş "Ders seç" gösteriyordu). Düzeltme: derin bağlantı işleme efekti artık `subjects` state'i dolana kadar bekliyor (bir `ref` ile yalnızca bir kez tetikleniyor). Tarayıcıda IndexedDB'den okunan gerçek ders/konu id'leriyle uçtan uca doğrulandı (ör. "Sayı Basamakları" konusundan Planla'ya basınca panel TYT/Matematik/Sayı Basamakları ön-seçili açıldı). Geçersiz/silinmiş `subjectId`/`topicId` durumunda panel çökmeden boş seçime dönüyor (kod incelemesiyle doğrulandı: `topics` yüklendikten sonra mevcut olmayan bir `topicId` otomatik temizleniyor).
- **İkinci bir gerçek hata bulundu ve düzeltildi (React StrictMode):** `QuickPlanPanel`'de "ders değişince konuyu sıfırla ama ilk (deep-link) yüklemede dokunma" mantığı başta bir `isFirstRender` boolean `ref`'iyle yazılmıştı. React StrictMode geliştirme modunda efektleri iki kez çalıştırdığından (`bootstrapService`'te de belgelenen bilinen proje tuzağı), bu bayrak ikinci simüle-mount'ta yanlışlıkla "artık ilk render değil" sanıp konu seçimini siliyordu — tarayıcıda canlı olarak yeniden üretildi (deep-link'ten gelen konu, panel açılır açılmaz kayboluyordu). Düzeltme: bayrak yerine önceki `subjectId` değerini bir `ref`'te tutup gerçek değişimi karşılaştıran bir desene geçildi (`prevSubjectIdRef`); bu StrictMode'un çift-çalıştırmasına karşı doğal olarak dayanıklı. Düzeltme sonrası tekrar canlı doğrulandı.
- **Günlük özet:** `PlannerPage`'in günlük görünümünde, seçilen güne ait `StudyTask` kayıtlarından hesaplanan (sahte veri yok) 4 küçük sayaç eklendi: toplam görev, tamamlanan, planlanan dakika, planlanan soru. Görev kartlarına ders adının yanına sınav türü rozeti ve saatsiz görevler için "Gün içinde" düşük öncelikli metni eklendi; tamamlanan görevler zaten sakin yüzey + onay ikonuyla gösteriliyordu (değişmedi).
- **Hızlı şablonlar:** Plan ekranının üst kısmına 4 sabit şablon çipi eklendi (`src/constants/quickPlan.ts`): "45 dk konu çalış", "40 soru çöz", "30 dk tekrar yap", "Branş denemesi çöz". Şablona basmak yalnızca Hızlı Planla panelini ilgili çalışma türü/süre/soru alanları önceden dolu şekilde açıyor; ders/konu/gün seçili bırakılmıyor ve Kaydet'e basılmadan görev oluşmuyor.
- **Son kullanılan seçim hafızası:** `src/features/planner/quickPlanMemory.ts` içinde kalıcı olmayan (sayfa yenilemesinde sıfırlanan), modül seviyesinde basit bir değişken — panel yeniden açıldığında son kullanılan sınav türü/dersi hatırlatıyor. Yeni bir store/context sistemi kurulmadı.
- **Nisa'ya özel UI cilası (ölçülü):** `EmptyState` ikonunun arkasına çok hafif bulanık bir parıltı eklendi (tüm boş durumlarda tutarlı, tek noktadan). Dashboard karşılama kartına ikinci, küçük bir mercan tonlu dekoratif leke eklendi (mevcut mor lekenin simetriği, düşük opaklık). Kişisel metinler eklendi: Hızlı Planla panel açıklaması "Bugünün planını birlikte hazırlayalım.", şablon satırı altyazısı "Küçük adımlar da ilerlemedir.", görev kaydedildiğinde 3 saniyelik geçici bir başarı bandı "Harika, bugünkü planına bir çalışma daha eklendi." (brief'te verilen örnek metinler, aynen kullanıldı). Mevcut günlük kişisel mesaj sistemi (`dailyMessage.ts`) hiç değiştirilmedi. Yeni animasyon/kütüphane eklenmedi; yalnızca mevcut `press`/`animate-rise` sınıfları ve statik CSS blur kullanıldı.
- **Yeni ortak component:** `QuickChoiceChip` (`src/components/ui/QuickChoiceChip.tsx`) — Hızlı Planla ve şablon satırında kullanılan tek tip dokunmatik seçim çipi. Büyük bir UI framework veya kapsamlı soyutlama oluşturulmadı.
- **Kod tabanı tarafında değişmeyenler:** IndexedDB şeması/`DB_VERSION` değişmedi (konular zaten mevcut `topics` store'una ekleniyor, migration gerekmedi), backend/API/yeni bağımlılık eklenmedi, mevcut ayrıntılı görev formu (`TaskFormPanel`) davranışı bozulmadı, mevcut tüm özellikler (sayaç, denemeler, yanlışlar, tekrarlar, hedefler, kaynaklar, notlar, istatistikler, ayarlar, yedekleme, sıfırlama, Bölüm Keşfi, PWA, tema) test edilmeden dokunulmadı.
- **Doğrulama:** `npx tsc -b` hatasız, **30/30 test geçiyor** (yeni büyük test paketi yazılmadı, brief bunu istemedi; seed idempotency ve deep-link davranışı tarayıcıda canlı doğrulandı), `npm run build` başarılı (518,24 kB JS / 38,06 kB CSS, PWA v1.3.0, 17 precache girdisi). Tarayıcıda gerçek geliştirme verisiyle (`localhost:5173`) uçtan uca smoke test yapıldı: TYT ve AYT kataloğu doğru oluştu, TYT/AYT Matematik karışmadı, seed ikinci çalışmada çoğalma yapmadı, mevcut manuel konu ve durumu korundu, Planla butonu doğru ders/konuyla paneli açtı, otomatik başlık ve kullanıcı düzenlemesi doğru çalıştı, Bugün/Hafta sonu tarihleri doğru hesaplandı, süre/soru hızlı seçimleri çalıştı, görev IndexedDB'ye kaydedildi ve günlük özete yansıdı, 375 px ve masaüstünde yatay taşma yok, açık/koyu tema bozulmadı, konsolda hiç hata oluşmadı. Test sırasında oluşturulan tek örnek görev ("TYT Matematik çalış") gerçek veriye karışmaması için silindi; katalog konuları ise kalıcı ürün verisi olduğu için silinmedi.
- **Bilinen sorunlar (bu geçişten):** Hızlı Planla'da sınav türü seçimi brief'in istediği iki seçenek (TYT/AYT) yerine üç seçenek (TYT/AYT/Özel) olarak uygulandı — bilinçli bir kapsam genişletmesi, aksi hâlde kullanıcının önceden var olan özel dersleri Hızlı Planla'dan planlanamaz hâle gelirdi. Branş denemesi/genel deneme/yanlış analizi otomatik başlık kalıpları brief'te örneklenmediği için makul bir varsayımla tasarlandı, resmî bir örnekle doğrulanmadı. Ayrılma durumu eki (-den/-dan/-ten/-tan) basit bir ünlü uyumu kuralına dayanıyor; Türkçenin tüm istisnalarını (ünlüyle biten sözcüklerdeki kaynaştırma harfi gibi) kapsamıyor. Son kullanılan ders hafızası sayfa yenilemesinde sıfırlanıyor (bilinçli, kalıcı depoya yazılmıyor). Genel deneme/branş denemesi görev türlerinde de Hızlı Planla akışı hâlâ bir ders seçimi istiyor (mevcut ayrıntılı formla tutarlı, ancak "genel deneme" kavramsal olarak tek derse bağlı değildir).
